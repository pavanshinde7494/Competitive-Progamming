...
void quickSort(ll arr[], ll left , ll right) {
    ...
    ll pivote = arr[left];
    ll pivoteIndex = left;
    ll low = left + 1 , high = right - 1;

    while (low < high) {
        while (pivote >= arr[low] && low <= right )low++;
        while (pivote < arr[high] && high >= left) high--;
        if (low < high) {
    ...
    swap(arr[pivoteIndex] , arr[high]);
    pivoteIndex = high;

    quickSort(arr , left , pivoteIndex);
